...
export default class Database extends Component {
  ...
  componentDidMount() {
    axios.get("/private/quizdata/databasequestions",{
      auth:{
      ...
      .then(response => {
        ...
        this.setState({ Databasequestions: response.data })
      }).catch(console.log)
  }
  ...
  f1() {
    score = 0
    this.state.Databasequestions.map(w => {
    ...
    this.setState({Subject:"DATABASE"})
  ...
  render() {
    return (
      ...
      <Timer />
        ...
        {this.state.Databasequestions.map((e) => (
